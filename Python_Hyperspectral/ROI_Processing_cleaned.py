#!/usr/bin/env python3
"""
Hyperspectral ROI Analysis Pipeline

This script processes manual ROI data from Fiji to perform segmentation-based
spectral analysis on hyperspectral CZI image data. It extracts and classifies
spectral signatures from manually selected ROIs and generates comprehensive
visualizations with consistent color mapping.
...
def visualize_roi_mask(labeled_mask, segmentation_input, spectral_df, num_labels, 
    ...
    ax_ref.set_title('Reference Image Channel 15 w/ Max Z Projection')
    ax_ref.axis('off')
...
def main():
    ...
    try:
        ...
        # Visualizations
        visualize_roi_mask(labeled_mask, segmentation_input, spectral_df, 
                          num_labels, vmin_val, vmax_val)
        plot_individual_channels(spectral_df, num_channels)
        plot_spectral_signatures(spectral_df, num_channels, num_labels)
        
        # Clustering analysis
        spectral_df = perform_clustering(spectral_df, num_channels, labeled_mask, 
                                       segmentation_input)
